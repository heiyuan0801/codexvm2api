...
在代理管理中测试代理会通过代理请求出口 IP 检测接口，并尽力保存真实出口 IP、国家/地区、城市和 IANA 时区。
容器卡片和详情会显示最近一次检测结果；把代理绑定到槽位后，检测到的时区会自动同步到该槽位的软件身份，
下一轮对账会按新的 `TZ` 重建运行环境。位置接口不可用时仍保留连通性测试结果，原有手工位置也不会因失败被清除。

每个槽位的 hostname、machine-id、installation-id 和 HOME 卷在槽位记录中持久保存，容器重建不会变化，
只有删除槽位后重新创建才会生成新值。sidecar 同时固定 `LANG=C.UTF-8`、`LC_ALL=C.UTF-8`、HOME 路径和身份文件权限，
并使用固定 digest 的基础镜像。TPM、SMBIOS、磁盘序列号属于宿主机/虚拟机硬件身份，普通 Docker 容器不提供可靠的模拟能力，
因此槽位只保证可核验的软件身份和代理出口信息，不把它们伪装成真实硬件证明。
